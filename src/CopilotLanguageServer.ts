...
import { InlineCompletionList } from "vscode-languageserver-types"
...
class CopilotLanguageServer {
    ...
    constructor(commonSyntaxes: string[]) {
        ...
        nova.config.observe("besya.copilot.node-path", (path: string) => {
            if (path == "") {
                new Notification("node has not been found").show()
                return
            }
            this.nodePath = path
            this.start()
        ...
        nova.config.observe("besya.copilot.language-server-path", (path: string) => {
            if (path == "") {
    ...
    async signIn() {
        ...
        try {
            let response = (await this.languageClient.sendRequest("signIn", {})) as SignInResponse
            let notification = new Notification(response.userCode)
            notification.action("Cancel", () => {})
            notification.action("Sign in with Github", async () => await this.auth(response))
            notification.show()
        } catch (error) {
            new Notification(error as string).show()
        }
