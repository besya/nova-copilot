import type CopilotLanguageServer from './CopilotLanguageServer'
  ...
  constructor(languageServer: CopilotLanguageServer) {
    ...
    nova.config.observe(
      ...
      (enabled: boolean) => {
        ...
        new Notification(
          `Inline Completions has been ${enabled ? 'enabled' : 'disabled'}`,
        ).show()
      },
      this,
    )
    ...
    this.register = nova.workspace.onDidAddTextEditor(
      (editor: TextEditor) =>
        this.completers.push(
          new OnChangeCompleter(
            editor,
            this.languageServer as CopilotLanguageServer,
          ),
        ),
      this,
    )
